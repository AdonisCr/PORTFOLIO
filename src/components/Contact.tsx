  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (serviceId === 'YOUR_SERVICE_ID' || 
        ...
        throw new Error('⚠️ Vous devez d\'abord configurer vos identifiants EmailJS dans Contact.tsx (lignes 27-29)');
      }
      ...
      let errorMessage = 'Une erreur est survenue. Veuillez réessayer ou me contacter directement par email.';
      ...
      if (error.message && error.message.includes('identifiants EmailJS')) {
        ...
        errorMessage = '❌ Erreur 400: Vérifiez que vos Service ID, Template ID et Public Key sont corrects dans le code.';
      } else if (error.status === 401) {
        errorMessage = '❌ Erreur 401: Votre Public Key est invalide.';
      } else if (error.status === 404) {
        errorMessage = '❌ Erreur 404: Template ou Service introuvable. Vérifiez les IDs.';
      }
  ...
  return (
    <section id="contact" className={`py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden ${
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className={`absolute -top-40 -right-40 w-60 sm:w-80 h-60 sm:h-80 rounded-full blur-3xl opacity-20 ${
          isDarkMode ? 'bg-purple-500' : 'bg-purple-300'
        }`}></div>
        <div className={`absolute -bottom-40 -left-40 w-60 sm:w-80 h-60 sm:h-80 rounded-full blur-3xl opacity-20 ${
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="text-center mb-12 sm:mb-16">
          ...
          <div className={`w-20 sm:w-24 h-1 mx-auto mb-6 sm:mb-8 ${
            isDarkMode 
          ...
          <div>
            ...
            <div className="space-y-4 sm:space-y-6">
              <div className="flex items-center space-x-3 sm:space-x-4">
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  ...
                  <Mail className={isDarkMode ? 'text-purple-400' : 'text-purple-600'} size={18} />
                </div>
                <div className="min-w-0">
                  <h4 className={`font-semibold text-sm sm:text-base ${
                    isDarkMode ? 'text-white' : 'text-gray-900'
                  }`}>Email</h4>
                  <p className={`text-xs sm:text-sm break-all ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              ...
              <div className="flex items-center space-x-3 sm:space-x-4">
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  ...
                  <Phone className={isDarkMode ? 'text-blue-400' : 'text-blue-600'} size={18} />
                </div>
                <div>
                  <h4 className={`font-semibold text-sm sm:text-base ${
                    isDarkMode ? 'text-white' : 'text-gray-900'
            ...
            <div className="mt-8 sm:mt-12">
              ...
              <div className="flex space-x-3 sm:space-x-4">
                <a href="#" className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center transition-all duration-300 ${
                  isDarkMode 
            ...
            {/* Availability */}
            <div className={`mt-8 sm:mt-12 p-4 sm:p-6 backdrop-blur-sm rounded-xl border ${
              isDarkMode 
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              ...
              <button
                ...
                className={`w-full text-white py-2.5 sm:py-3 px-6 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center space-x-2 text-sm sm:text-base ${
                  ...
                  isDarkMode
                    ? 'bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 hover:shadow-lg hover:shadow-purple-500/50'
                    : 'bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 hover:shadow-lg hover:shadow-purple-600/50'
                }`}
              >
                {isLoading ? (
                  <>
